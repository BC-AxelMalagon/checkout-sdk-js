...
export default class StripeV3PaymentStrategy implements PaymentStrategy {
    ...
    private _shouldUseIndividualCardFields?: boolean;
    ...
    async initialize(options: PaymentInitializeOptions): Promise<InternalCheckoutSelectors> {
        ...
        const { initializationData: { stripePublishableKey, stripeConnectedAccount, shouldUseIndividualCardFields } } = paymentMethod;

        this._shouldUseIndividualCardFields = shouldUseIndividualCardFields;
    ...
    private _mountElement(methodId: string): Promise<StripeElement> {
        ...
        return new Promise((resolve, reject) => {
            ...
            switch (stripeElementType) {
                case StripeElementType.CreditCard:
                    if (this._shouldUseIndividualCardFields && isIndividualCardElementOptions(options)) {
    ...
    private _mapStripePaymentData(element: StripePaymentMethodType, arg2?: any): StripeConfirmPaymentData {
        ...
        result = {
            payment_method: {
                [element]: element === StripePaymentMethodType.CreditCard && this._shouldUseIndividualCardFields ? this._getStripeCardElements()[0] : this._getStripeElement(),
